studentNew2 <- read.fwf("~/Downloads/INT_Stu06_Dec07.txt", len)
colnames(studentNew2) <- colnames(student2006)
...
for (ind in indy) {
  key <- as.character(studentNew[,toupper(s1[ind])])
  key2 <- paste0(sapply(max(nchar(key)) - nchar(key), function(ii) substr("0000", 1, ii)), key)
  studentNew[,toupper(s1[ind])] <-   factor(slownik[[s2[ind]]][key2],
                                             levels=slownik[[s2[ind]]])
}

student2006 = studentNew
s1[ind]
